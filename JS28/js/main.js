document.getElementsById("phoneNum").addEventListener("input", (event) => {
...
document.getElementById("textForm").addEventListener("submit", (event) => {
    ...
    const regex = / {2,}/g;
    const newText = input.value.replaceAll(regex, " ").trim(); //trim is used to remove whitespace from the string
    console.log(newText);
    const encodedInputText = encodeURI(input.value); //encodeURL is used to encode a string to Uniform Resource Locator(URL)
